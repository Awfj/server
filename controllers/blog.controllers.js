...
export const bookmarkBlog = async (req, res) => {
  try {
    ...
    if (isBookmarked) {
      ...
      return res
        .status(200)
        .json({ message: "Blog unbookmarked successfully" });
    } else {
...
export const getUserBookmarkedBlogs = async (req, res, next) => {
  try {
    ...
    const user = await User.findById(userId).populate({
      ...
      match: query ? { title: new RegExp(query, "i") } : {}, // Add title search
      populate: {
        path: "author",
        select:
          "personal_info.profile_img personal_info.fullname personal_info.username -_id",
...
export const userBookmarksCount = async (req, res) => {
  try {
    ...
    const bookmarksCount = user.bookmarkedPosts
